...
{
    public class Dim9DoubleNumpyWrapper : INumpyArrayWrapper<double>
    {
        ...
        public Dim9DoubleNumpyWrapper(double[,,,,,,,,] array)
        {
            ...
            this.shape = new[]
            {
                ...
                array.GetLength(5), array.GetLength(6), array.GetLength(7), array.GetLength(8)
            };
        }
        ...
        public NumpyArray<double> WrapInnerSubsetArray(params int[] indices)
        {
            ...
            if (indices.Length == 1)
            {
                result = NumpyArrayFactory.From(
                    (double[,,,,,,,])NumpyArrayFactory.WrapInnerSubsetArray(array, indices));
            }
        ...
        {
            double[,,,,,,,,] result = new double[shape[8], shape[7], shape[6], shape[5], shape[4], shape[3], shape[2],
                shape[1], shape[0]];
                                ...
                                for (int d5 = 0; d5 < this.shape[5]; d5++)
                                {
                                    for (int d6 = 0; d6 < this.shape[6]; d6++)
                                    {
                                        for (int d7 = 0; d7 < this.shape[7]; d7++)
                                        {
                                            ...
                                            {
                                                result[d8, d7, d6, d5, d4, d3, d2, d1, d0] =
                                                    array[d0, d1, d2, d3, d4, d5, d6, d7, d8];
                                            }
                                        }
                                    }
                                }
