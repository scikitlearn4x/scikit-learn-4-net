...
{
    public class Dim10Int16NumpyWrapper : INumpyArrayWrapper<short>
    {
        private short[,,,,,,,,,] array;
        private int[] shape;
        ...
        public NumpyArray<short> WrapInnerSubsetArray(params int[] indices)
        {
            ...
            if (indices.Length == 1)
            {
                result = NumpyArrayFactory.From(
                    (short[,,,,,,,,])NumpyArrayFactory.WrapInnerSubsetArray(array, indices));
            }
        ...
        {
            short[,,,,,,,,,] result = new short[shape[9], shape[8], shape[7], shape[6], shape[5], shape[4], shape[3],
