...
{
    public class Dim10FloatNumpyWrapper : INumpyArrayWrapper<float>
    {
        private float[,,,,,,,,,] array;
        private int[] shape;
        ...
        public NumpyArray<float> WrapInnerSubsetArray(params int[] indices)
        {
            ...
            if (indices.Length == 1)
            {
                result = NumpyArrayFactory.From(
                    (float[,,,,,,,,])NumpyArrayFactory.WrapInnerSubsetArray(array, indices));
            }
        ...
        {
            float[,,,,,,,,,] result = new float[shape[9], shape[8], shape[7], shape[6], shape[5], shape[4], shape[3],
